...
const Hero = () => {
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        navigate(`/rooms?destination=${destination}&guests=${guests}`);
        scrollTo(0, 0);
    }

    return (
        <div
            className="flex flex-col items-start justify-center px-6 md:px-16 lg:px-24 xl:px-32 text-white h-screen bg-cover bg-center"
            style={{ backgroundImage: `url(${heroImage})` }}>
            <p className='bg-[#49B9FF]/50 px-3.5 py-1 rounded-full mt-20 text-xs md:text-sm'>The Ultimate Hotel Experience</p>
